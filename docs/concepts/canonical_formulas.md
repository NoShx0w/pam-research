Yes — this version is mathematically fine, but GitHub Markdown preview is the problem. The safest fix is to convert every display equation to fenced math blocks and keep inline symbols as $...$.

Here is a GitHub-safe version you can paste directly into canonical_formulas.md:
...
- In GitHub preview, ` ```math ` is usually the most reliable for block equations.
- Keep inline references like `$d_{\mathcal B}$`, `$Y_t^{(k)}$`, and `$\phi(\theta)$` inline, not in separate block math, unless they need visual emphasis.
If you want, I can also give you a **cleaned-up version with fewer one-line display equations** so the document reads a bit more smoothly.