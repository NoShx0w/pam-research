"""Canonical gradient-alignment stage for the PAM topology pipeline."""
    ...
    summary = pd.DataFrame(
        {
            ...
            "mean_alignment_high_lazarus": [
                pd.to_numeric(
                    work.loc[work["lazarus_group"] == "high", "gradient_cosine_alignment"],
                    errors="coerce",
                ).mean()
            ],
            "mean_alignment_low_lazarus": [
                pd.to_numeric(
                    work.loc[work["lazarus_group"] == "low", "gradient_cosine_alignment"],
                    errors="coerce",
                ).mean()
            ],
            "mean_abs_alignment_near_seam": [
                pd.to_numeric(
                    work.loc[
                        work["distance_to_seam"] <= work["distance_to_seam"].median(),
                        "gradient_cosine_alignment",
                    ],
                    errors="coerce",
                ).abs().mean()
            ],
