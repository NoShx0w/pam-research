"""Canonical scalar-curvature stage for the PAM geometry pipeline."""
...
def run_curvature(
    ...
    out_df.to_csv(outdir / "curvature_surface.csv", index=False)

    heatmap(K, r_vals, a_vals, "scalar curvature", outdir / "scalar_curvature.png")
    heatmap(
        np.log10(np.abs(K) + 1e-12),
        r_vals,
        a_vals,
        "log10 |curvature|",
        outdir / "log_abs_scalar_curvature.png",
    )
