"""Canonical scaled GE probe stage for the PAM operators layer."""
...
def annotate_path(path_df: pd.DataFrame, path_length: float) -> Dict:
    ...
    constraint_strength = (
        float(max_curvature_along_path / max(min_distance_to_seam, 1e-9))
        if pd.notna(min_distance_to_seam) and pd.notna(max_curvature_along_path)
        else float("nan")
    )
...
def run_scaled_probes(
    ...
    G = build_graph(edges)
    node_df = load_node_table(mds_csv, signed_phase_csv, curvature_csv, lazarus_csv)
    ...
    probes_df = sample_probe_pairs(node_df, n_pairs, seed)
    ...
    render_probe_cloud(node_df, seam, all_paths, outdir / "scaled_probe_cloud_on_mds.png", max_draw=max_draw)
