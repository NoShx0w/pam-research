"""Canonical GE probe stage for the PAM operators layer."""
...
def choose_probe_endpoints(df: pd.DataFrame) -> List[Dict]:
    ...
    if not pos.empty and not neg.empty:
        ...
        probes.append({
            "probe_id": "basin_to_basin_extreme",
    ...
    if len(pos) >= 2:
        ...
        probes.append({
            "probe_id": "same_phase_positive_control",
    ...
    if len(neg) >= 2:
        ...
        probes.append({
            "probe_id": "same_phase_negative_control",
...
def run_probes(
    ...
    df = load_nodes(mds_csv, signed_phase_csv, curvature_csv)
