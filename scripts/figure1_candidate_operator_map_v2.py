...
OUTFILE = OUTDIR / "figure1_candidate_operator_map_v2.png"
...
def draw_filaments(ax, X, Y, resp, seam_dist, n_samples: int = 40):
    ...
    for i, j in chosen:
        ...
        ax.plot([x1, x2], [y1, y2], color=color, alpha=alpha, linewidth=1.2, solid_capstyle="round")


def draw_active_seam_highlights(ax, X, Y, strength_norm, seam_dist):
    finite = np.isfinite(strength_norm)
    if not finite.any():
        return
    thresh = np.nanquantile(strength_norm[finite], 0.85)
    near_thresh = np.nanquantile(seam_dist[finite], 0.25)
    active_mask = (strength_norm >= thresh) & (seam_dist <= near_thresh)
    if np.any(active_mask):
        ax.contour(
            X, Y, active_mask.astype(float),
            levels=[0.5],
            colors="white",
            linewidths=2.4,
            alpha=0.18
...
def main():
    ...
    ax.contour(
        X, Y, phase_Z,
        levels=[-0.35, 0.0, 0.35],
        colors="white",
        linewidths=[0.45, 1.2, 0.45],
        alpha=0.45
    )

    if len(seam_pts) > 1:
        ax.plot(seam_pts[:, 0], seam_pts[:, 1], color="white", linewidth=1.4, alpha=0.8)

    draw_active_seam_highlights(ax, X, Y, strength_norm, seam_dist)
    ...
    if path_xy is not None:
        ...
        markers = [
            (peak_idx, "o", "gold", "1 peak"),
            (contact_idx, "o", "white", "2 seam"),
            (flip_idx, "*", "deepskyblue", "3 flip"),
        ]
