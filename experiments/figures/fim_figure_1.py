...
def render_figure(df: pd.DataFrame, outpath: Path) -> None:
    ...
    gs = gridspec.GridSpec(
        2,
        4,
        width_ratios=[1.0, 0.045, 1.0, 0.040],
    ...
    cbB.set_label("Distance to phase boundary", labelpad=8)
    cbB.ax.tick_params(labelsize=9)
    cbB.ax.yaxis.label.set_size(10)
    ...
    cbC.set_label("Log curvature", labelpad=8)
    cbC.ax.tick_params(labelsize=9)
    cbC.ax.yaxis.label.set_size(10)
    ...
    fig.subplots_adjust(top=0.79, left=0.07, right=0.95, bottom=0.08)
