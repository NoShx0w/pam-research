...
def classify_outcome(crossed: bool, entered_lazarus: bool, phase_flip_count: int, min_distance: float, graze_threshold: float) -> str:
...
def summarize_paths(paths: pd.DataFrame, metrics: pd.DataFrame | None, graze_threshold: float, corpus_link_map: dict[str, bool]) -> pd.DataFrame:
    group_col = infer_group_col(paths)
    work = paths.copy()
    work = safe_numeric(work, ["r", "alpha", "distance_to_seam", "scalar_curvature", "lazarus_score", "signed_phase", "step"])

    if metrics is not None:
        metrics = metrics.copy()
        metrics = safe_numeric(metrics, ["start_r", "start_alpha", "end_r", "end_alpha", "crosses_seam", "phase_flip_count", "min_distance_to_seam", "max_curvature_along_path", "lazarus_max"])
    ...
    for gid, g in work.groupby(group_col, sort=False):
        ...
        phase_flip_count = int(
            mrow.get("phase_flip_count", 0) if mrow is not None and "phase_flip_count" in mrow.index else
            max(0, int((phase_series.diff().fillna(0) != 0).sum())) if len(phase_series) else 0
        )

        min_distance = float(
            mrow.get("min_distance_to_seam", np.nan) if mrow is not None and "min_distance_to_seam" in mrow.index else
            g["distance_to_seam"].min(skipna=True) if "distance_to_seam" in g.columns else np.nan
        )

        max_lazarus = float(
            mrow.get("lazarus_max", np.nan) if mrow is not None and "lazarus_max" in mrow.index else
            g["lazarus_score"].max(skipna=True) if "lazarus_score" in g.columns else np.nan
        )

        max_curvature = float(
            mrow.get("max_curvature_along_path", np.nan) if mrow is not None and "max_curvature_along_path" in mrow.index else
            g["scalar_curvature"].max(skipna=True) if "scalar_curvature" in g.columns else np.nan
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Extract initial conditions -> outcomes from existing trajectory data.")
    ...
    metrics = pd.read_csv(args.metrics_csv) if Path(args.metrics_csv).exists() else None
    corpus_link_map = load_corpus_link_map(Path(args.corpora_py)) if args.corpora_py else {}

    ic_df = summarize_paths(paths, metrics, args.graze_threshold, corpus_link_map)
    ic_df.to_csv(outdir / "initial_conditions_outcomes.csv", index=False)

    outcome_summary = build_outcome_summary(ic_df, ["corpus", "r", "alpha"])
    outcome_summary.to_csv(outdir / "initial_conditions_outcome_summary.csv", index=False)

    link_summary = build_link_geometry_summary(ic_df)
    link_summary.to_csv(outdir / "link_geometry_summary.csv", index=False)

    print(outdir / "initial_conditions_outcomes.csv")
    print(outdir / "initial_conditions_outcome_summary.csv")
    print(outdir / "link_geometry_summary.csv")
