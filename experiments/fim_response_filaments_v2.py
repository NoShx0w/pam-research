...
def interpolate_surface(df: pd.DataFrame, grid_n: int = 220):
...
def nearest_surface_z(xv: np.ndarray, yv: np.ndarray, gx: np.ndarray, gy: np.ndarray, gz: np.ndarray) -> np.ndarray:
    xs = gx[0, :]
    ys = gy[:, 0]
...
def add_filaments(ax, active: pd.DataFrame, gx: np.ndarray, gy: np.ndarray, gz: np.ndarray, z_offset: float = 0.03) -> None:
    if active.empty:
        return
    ...
    zs = nearest_surface_z(xs, ys, gx, gy, gz) + z_offset
    ...
    for i, (_, row) in enumerate(active.iterrows()):
        ...
        z = zs[i]
        ax.plot([x0, x1], [y0, y1], [z, z], color="white", linewidth=1.7, alpha=0.92)


def add_quiver(ax, active: pd.DataFrame, gx: np.ndarray, gy: np.ndarray, gz: np.ndarray, z_offset: float = 0.03) -> None:
    if active.empty:
        return
...
def render(df: pd.DataFrame, seam: pd.DataFrame, paths: pd.DataFrame, outpath: Path, mode: str = "filaments") -> None:
    ...
    if not seam.empty:
        ...
        sz = nearest_surface_z(sx, sy, gx, gy, gz) + 0.025
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render a publication-ready grid-based response surface with sparse filaments or quiver.")
    ...
    parser.add_argument("--outdir", default="outputs/fim_response_filaments_v2")
    ...
    outpath = outdir / f"response_surface_{args.mode}_v2.png"
