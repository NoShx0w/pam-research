#!/usr/bin/env python3
"""
obs047_minimal_blend_baseline_stabilization.py

OBS-047 — Minimal-blend continuous baseline stabilization.

Purpose
-------
Compare a small set of continuous interpolation models under a narrow local
parameter sweep to determine the best seam-preserving continuous baseline.

Models compared:
- knn_avg_k8      : legacy OBS-044 baseline
- nearest_anchor  : most locally decisive steering
- top2_blend      : minimal local blending, current best compromise candidate
...
- same seeds across all configurations
- same support/consistency logic
- same family classifier
- no broad model expansion
- outputs kept inside a dedicated OBS-047 directory

Outputs
-------
Directory:
    outputs/obs047_minimal_blend_baseline_stabilization/

Files:
    baseline_stabilization_summary.csv
    baseline_stabilization_family_comparison.csv
    baseline_stabilization_phase_crossing_figure.png
    baseline_stabilization_release_cross_phase_figure.png
    baseline_stabilization_seam_contact_figure.png
    baseline_stabilization_summary.txt

Per-configuration subdirectories:
    outputs/obs047_minimal_blend_baseline_stabilization/<config_label>/
...
DEFAULT_OUTDIR = Path("outputs/obs047_minimal_blend_baseline_stabilization")
...
FAMILY_ORDER = ["seam_hugging", "release_directed", "short_trapped", "mixed"]


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="OBS-047 minimal-blend continuous baseline stabilization.")
    p.add_argument("--input", type=Path, default=DEFAULT_INPUT)
    p.add_argument("--seam-nodes-csv", type=Path, default=DEFAULT_SEAM_NODES)
    p.add_argument("--outdir", type=Path, default=DEFAULT_OUTDIR)

    p.add_argument(
        "--modes",
        nargs="+",
        default=["knn_avg_k8", "nearest_anchor", "top2_blend"],
        help="Interpolation modes to compare.",
    )
    p.add_argument(
        "--support-radius-scales",
        nargs="+",
        type=float,
        default=[3.5, 4.0],
    )
    ...
    p.add_argument("--n-seeds", type=int, default=26)
...
def compute_response_eigensystem(df: pd.DataFrame) -> pd.DataFrame:
    vals_1, vec1_x, vec1_y, eig_valid = [], [], [], []
...
def align_vectors(local_vecs: np.ndarray, anchor: np.ndarray) -> np.ndarray:
    aligned = []
    for v in local_vecs:
        vv = v.copy()
...
def parse_mode(mode: str) -> tuple[str, int]:
    if mode.startswith("knn_avg_k"):
        return "knn_avg", int(mode.split("k")[-1])
    if mode == "nearest_anchor":
        return "nearest_anchor", 1
    if mode == "top2_blend":
        return "top_blend", 2
    raise ValueError(f"Unsupported mode for OBS-047: {mode}")


def interpolate_vector_mode(
    z: np.ndarray,
    xy: np.ndarray,
    vecs: np.ndarray,
    mode: str,
    ...
    aligned = align_vectors(local_vecs, anchor)

    if family == "nearest_anchor":
        vbar = aligned[0].copy()
        cons = 1.0
        support_radius = float(local_dist[0])
        neighbor_count = 1
    else:
...
def integrate_continuous_path(
    ...
    seam_vals, phase_vals, laz_vals, mismatch_vals, hol_vals = [], [], [], [], []
    path_length = 0.0
    ...
    for step in range(max_steps + 1):
        interp = interpolate_vector_mode(z, xy, vecs, mode=mode, prev_dir=prev_dir)
        ...
        seam_val = interpolate_scalar(z, xy, seam_values, scalar_k)
        phase_val = interpolate_scalar(z, xy, phase_values, scalar_k)
        laz_val = interpolate_scalar(z, xy, laz_values, scalar_k)
        mismatch_val = interpolate_scalar(z, xy, mismatch_values, scalar_k)
        hol_val = interpolate_scalar(z, xy, hol_values, scalar_k)
...
def plot_paths(points_df: pd.DataFrame, nodes_df: pd.DataFrame, outpath: Path) -> None:
    ...
    ax.set_xlabel("mds1")
    ax.set_ylabel("mds2")
    ax.set_title("OBS-047 baseline stabilization")
...
def plot_quiver(nodes_df: pd.DataFrame, outpath: Path, stride: int) -> None:
    ...
    ax.quiver(
        qdf["mds1"], qdf["mds2"], qdf["eigvec1_x"], qdf["eigvec1_y"],
        angles="xy", scale_units="xy", scale=1.0, width=0.0025, alpha=0.9
    )
    ax.set_xlabel("mds1")
    ax.set_ylabel("mds2")
    ax.set_title("OBS-047 response direction anchors")
    ax.set_aspect("equal", adjustable="datalim")
    fig.tight_layout()
    fig.savefig(outpath, dpi=180)
    plt.close(fig)
...
def plot_metric(df: pd.DataFrame, metric: str, ylabel: str, outpath: Path) -> None:
    plot_df = df.copy().sort_values(["mode", "support_radius_scale", "step_size_scale"])
...
def write_summary_text(summary_df: pd.DataFrame, outpath: Path) -> None:
    best_release = summary_df.sort_values(
        ["release_directed_cross_phase_share", "phase_sign_change_share", "seam_contact_share"],
        ascending=[False, False, False],
    ).iloc[0]
    best_phase = summary_df.sort_values(
        ["phase_sign_change_share", "release_directed_cross_phase_share", "seam_contact_share"],
        ascending=[False, False, False],
    ).iloc[0]

    text = f"""OBS-047 minimal-blend baseline stabilization summary

Configurations tested: {len(summary_df)}

Best release-directed cross-phase recovery:
- config_label: {best_release['config_label']}
- mode: {best_release['mode']}
- support_radius_scale: {best_release['support_radius_scale']}
- step_size_scale: {best_release['step_size_scale']}
- release_directed_cross_phase_share: {best_release['release_directed_cross_phase_share']:.6f}
- phase_sign_change_share: {best_release['phase_sign_change_share']:.6f}
- seam_contact_share: {best_release['seam_contact_share']:.6f}

Best overall phase crossing:
- config_label: {best_phase['config_label']}
- mode: {best_phase['mode']}
...
def main() -> None:
    ...
    configs = list(itertools.product(args.modes, args.support_radius_scales, args.step_size_scales))
    for mode, support_radius_scale, step_size_scale in configs:
        config_label = f"{mode}_sr{str(support_radius_scale).replace('.', 'p')}_ss{str(step_size_scale).replace('.', 'p')}"
        ...
        summary_df = pd.DataFrame([{
            ...
            "seam_nodes_csv": str(args.seam_nodes_csv),
        ...
        summary_rows.append(sweep_row)

        family_summary_df["config_label"] = config_label
        family_frames.append(family_summary_df)

        support_region_df = points_df[["trajectory_id", "step", "x", "y", "support_radius", "angular_consistency", "neighbor_count"]].copy()
    ...
    family_out = pd.concat(family_frames, ignore_index=True)

    summary_path = args.outdir / "baseline_stabilization_summary.csv"
    family_path = args.outdir / "baseline_stabilization_family_comparison.csv"
    txt_path = args.outdir / "baseline_stabilization_summary.txt"

    summary_out.to_csv(summary_path, index=False)
    family_out.to_csv(family_path, index=False)
    write_summary_text(summary_out, txt_path)

    plot_metric(summary_out, "phase_sign_change_share", "phase crossing share", args.outdir / "baseline_stabilization_phase_crossing_figure.png")
    plot_metric(
        summary_out,
        "release_directed_cross_phase_share",
        "release-directed cross-phase share",
        args.outdir / "baseline_stabilization_release_cross_phase_figure.png",
    )
    plot_metric(summary_out, "seam_contact_share", "seam contact share", args.outdir / "baseline_stabilization_seam_contact_figure.png")
