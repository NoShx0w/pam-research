#!/usr/bin/env python3
"""
obs046_interpolation_model_sensitivity.py

OBS-046 — Interpolation-model sensitivity in continuous response-flow reconstruction.

Purpose
-------
Compare local interpolation rules for the OBS-044 continuous response-flow
solver, testing whether the under-recovery of cross-phase release is caused by
over-smoothing in the local interpolation model rather than by the support
envelope itself.

This study is intentionally conservative:
- support radius is held fixed by default
- step size is held fixed by default
- only interpolation mode / local neighborhood size changes
- route-family analysis is included for each configuration

Default comparison set
----------------------
- knn_avg_k8   : OBS-044 baseline
- knn_avg_k5   : smaller-k averaging
- knn_avg_k3   : smaller-k averaging
...
-------
Directory:
    outputs/obs046_interpolation_model_sensitivity/

Files:
    interpolation_model_sensitivity_summary.csv
    interpolation_model_family_comparison.csv
    interpolation_model_phase_crossing_figure.png
    interpolation_model_release_cross_phase_figure.png
    interpolation_model_seam_contact_figure.png
    interpolation_model_summary.txt

Per-configuration subdirectories:
    outputs/obs046_interpolation_model_sensitivity/<config_label>/
...
DEFAULT_OUTDIR = Path("outputs/obs046_interpolation_model_sensitivity")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="OBS-046 interpolation-model sensitivity.")
    ...
    p.add_argument("--support-radius-scale", type=float, default=3.5)
    p.add_argument("--step-size-scale", type=float, default=0.15)
    ...
    p.add_argument(
        "--modes",
        nargs="+",
        default=["knn_avg_k8", "knn_avg_k5", "knn_avg_k3"],
...
def plot_paths(points_df: pd.DataFrame, nodes_df: pd.DataFrame, outpath: Path) -> None:
    ...
    ax.set_xlabel("mds1")
    ax.set_ylabel("mds2")
    ax.set_title("OBS-046 interpolation model sensitivity")
...
def plot_quiver(nodes_df: pd.DataFrame, outpath: Path, stride: int) -> None:
    ...
    ax.set_xlabel("mds1")
    ax.set_ylabel("mds2")
    ax.set_title("OBS-046 response direction anchors")
    ax.set_aspect("equal", adjustable="datalim")
    fig.tight_layout()
    fig.savefig(outpath, dpi=180)
    plt.close(fig)
...
def write_summary_text(summary_df: pd.DataFrame, outpath: Path) -> None:
    ...
    best_release = summary_df.sort_values("release_directed_cross_phase_share", ascending=False).iloc[0]

    text = f"""OBS-046 interpolation model sensitivity summary
...
def main() -> None:
    ...
    for mode in args.modes:
        ...
        scalar_k = 8  # keep scalar interpolation fixed for comparability
    ...
    summary_path = args.outdir / "interpolation_model_sensitivity_summary.csv"
    family_path = args.outdir / "interpolation_model_family_comparison.csv"
    txt_path = args.outdir / "interpolation_model_summary.txt"
    ...
    plot_metric(summary_out, "phase_sign_change_share", "phase crossing share", args.outdir / "interpolation_model_phase_crossing_figure.png")
    plot_metric(
        summary_out,
        "release_directed_cross_phase_share",
        "release-directed cross-phase share",
        args.outdir / "interpolation_model_release_cross_phase_figure.png",
    )
    plot_metric(summary_out, "seam_contact_share", "seam contact share", args.outdir / "interpolation_model_seam_contact_figure.png")
