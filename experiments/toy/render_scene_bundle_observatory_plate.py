#!/usr/bin/env python3
"""
render_scene_bundle_observatory_plate.py

Render a canonical 2D observatory-style plate using only the OBS-022 scene bundle.
"""
...
class Config:
    ...
    outdir: str = "outputs/obs022_scene_bundle_plate"
...
def summarize_representative_routes(routes: pd.DataFrame) -> pd.DataFrame:
    ...
    return out.sort_values(
        ["route_class", "order", "mean_lazarus", "n_steps"],
...
def write_summary_text(
    ...
    lines = [
        "=== Scene Bundle Observatory Plate Summary ===",
        "",
        "Bundle counts",
        f"n_nodes={len(nodes)}",
        f"n_edges={len(edges)}",
    ...
    for _, row in fam_summary.iterrows():
        lines.append(f"  {row['path_family']}: n_paths={int(row['n_paths'])}, share={float(row['share_paths']):.4f}")
...
def draw_main_panel(
    ...
    if len(hubs_draw):
        ...
        ax.scatter(hubs_draw["mds1"], hubs_draw["mds2"], s=18, c="black", zorder=7)

    ax.text(
        0.02,
        0.97,
        "phase manifold\nlarger circles = higher routing traffic",
...
def draw_diag_panel(ax, fam_summary, nodes, edges, seam, hubs, glyphs, routes) -> None:
    ax.axis("off")
    ...
    text = (
        ...
        f"routing hubs: {len(hubs)}\n"
        f"glyph anchors: {n_glyphs}\n"
        f"branch-away paths: {n_branch}\n\n"
        f"dominant family: {dominant_family}\n\n"
        "gold   = stable_seam_corridor\n"
        "crimson= reorganization_heavy\n"
        "cyan   = seam-exit / branch-away\n"
        "black  = seam / routing hubs"
    )
...
def draw_status_strip(ax, nodes, edges, seam, fam_summary) -> None:
    ax.axis("off")
    ...
    status = (
        f"OBS-022 scene bundle   •   nodes {len(nodes)}   •   edges {len(edges)}   •   "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render 2D observatory plate from OBS-022 scene bundle.")
    ...
    rep_summary = summarize_representative_routes(routes)
    ...
    side = gs[0:2, 1].subgridspec(2, 1, height_ratios=[0.70, 1.30], hspace=0.12)
    ax_family = fig.add_subplot(side[0, 0])
    ax_diag = fig.add_subplot(side[1, 0])
    ...
    fig.suptitle("PAM Observatory — bundle-rendered canonical plate", fontsize=20)

    png_path = outdir / "scene_bundle_observatory_plate.png"
    txt_path = outdir / "scene_bundle_observatory_plate_summary.txt"

    fig.savefig(png_path, dpi=220)
    plt.close(fig)

    write_summary_text(txt_path, fam_summary, rep_summary, hubs, nodes, edges, seam)
